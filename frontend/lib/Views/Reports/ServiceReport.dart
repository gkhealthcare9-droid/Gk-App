...
class ServiceRepotScreen extends StatefulWidget {
  const ServiceRepotScreen({super.key});

  @override
  State<ServiceRepotScreen> createState() => _ServiceRepotScreenState();
}

class _ServiceRepotScreenState extends State<ServiceRepotScreen> {
  ...
  Future<void> fetchHospital(String unique) async {
    ...
    if (_customerController.customer.value.id != null) {
      ...
      await _productController.fetchCustomerProducts(
        _customerController.customer.value.id!,
      );
    }
  }
  ...
  Future<Uint8List> _generateAndSavePdf() async {
    ...
    return await ServiceReport.generateServicereport(
                            ...
                            Expanded(
                              child: Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton<String>(
                                    isExpanded: true,
                                    hint: Text('Complaint from'),
                                    value: selectedEmployeeName,
                                    items:
                                        _customerController.employees.map((e) {
                                          return DropdownMenuItem<String>(
                                            value: e.name,
